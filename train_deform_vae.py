...
from augment_deform_vae import Augment
...
def train_and_validate(config):
    ...
    if config.bn_num == 2:
        target_net = get_model(config, num_class(config.dataset), bn_types=['base', 'deform'])
    else:
...
def train_epoch_two_bns(trainloader, model, epoch, config):
    ...
    for i, (input_list, target) in enumerate(trainloader):
        ...
        losses_adv.update((loss_aug * (-model.args.adv_weight_deform)).item(), input.size(0))
